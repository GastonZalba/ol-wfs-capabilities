...
const SCALAR_CAPABILITIES_110_PARSERS = makeStructureNS(NAMESPACE_URIS, {
  ...
  ComparisonOperators: makeObjectPropertySetter(readTagList),
...
function readNamedOnly(node) {
  return node.getAttribute('name') || readString(node) || undefined;
}
...
function readJoinedList(node, separator = ', ') {
  const arr = readString(node).split(separator);
  if (!arr || !arr.length) {
    return undefined;
  }
  return arr;
